...
from dgl.transform import partition_graph_with_halo, metis_partition_assignment
...
def bias_metis_partition(num_parts, g, sample_length):
    adjncy = xadj_adjncy(sample_length)
...
def improved_partition_graph(g, num_parts, sample_length, out_path, reshuffle, partition_data, xadj, adjncy, eweights, graph_name='test', balance_ntypes=None, balance_edges=False, num_hops=1, return_mapping=True):
    # degrees = (g.in_degrees()).tolist()
    # eweights = []
    ...
    #     size += 1
    ##adjncy = xadj_adjncy(sample_length)
    ## partition_data = bias_metis_partition(num_parts, g, sample_length)
    # n_cuts, membership = pymetis.part_graph(num_parts, adjacency=None, xadj=xadj, adjncy=adjncy, vweights=None,
    #                                         eweights=eweights, recursive=False)
    ...
    if num_parts == 1:
        ...
        # node_parts = th.tensor(a, dtype=torch.int64)
        ...
        if num_parts > 1:
            ...
            for ntype in g.ntypes:
                ...
                for i in parts:
                    ...
                    # node_map_val[ntype].append([int(partition_data[0]),int(partition_data[-1])+1])
    ...
    for i in range(num_parts):
        ...
        local_edges = generate_edges(g_edge)
        ...
        print()
        # print("Node in this partition ",partition_sets)
        # print("unique node", unique_nodes)
        # print("Halo Node", Halo_node)
        # print("local_node", unique_nodes - Halo_node)
        # print("source id", src_ids)
        # print("destination id", dst_ids)
        # print("EDGES", local_edges)

        #convert into networkx
        # G = nx.Graph(edges)
        # print("unique edges",G.to_undirected().size())
